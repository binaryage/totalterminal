...
-(NSDictionary*) objectValue {
    ...
    if ((keyCombo.code == ShortcutRecorderEmptyCode) || (keyCombo.flags == ShortcutRecorderEmptyFlags)) return nil;
...
-(Class) valueClassForBinding:(NSString*)binding {
    if ([binding isEqualToString:@"value"]) return [NSDictionary class];
...
-(void) shortcutRecorderCell:(SRRecorderCell*)aRecorderCell keyComboDidChange:(KeyCombo)newKeyCombo {
    if ((delegate != nil) && [delegate respondsToSelector:@selector(shortcutRecorder:keyComboDidChange:)]) {
        [delegate shortcutRecorder:self keyComboDidChange:newKeyCombo];                                                                                                     // propagate view changes to binding (see http://www.tomdalling.com/cocoa/implementing-your-own-cocoa-bindings)
    }
    ...
    if (bindingOptions != nil) {
        ...
        if (NilOrNull(transformer)) {
            ...
            if (!NilOrNull(transformerName)) transformer = [NSValueTransformer valueTransformerForName:transformerName];
        }

        if (!NilOrNull(transformer)) {
            if ([[transformer class] allowsReverseTransformation])
                value = [transformer reverseTransformedValue:value];
            else NSLog(@"WARNING: value has value transformer, but it doesn't allow reverse transformations in %s", __PRETTY_FUNCTION__);
        }
    }
